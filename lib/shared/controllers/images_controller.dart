...
class ImagesController {
  ...
  Future<List> getImagesFirebase(userName) async {
  ...
  Future<String> getImagesLocal(BuildContext context, userName) async {
    final instance = await SharedPreferences.getInstance();
    if (instance.containsKey("imagesList")) {
      final imagesJson = instance.get("imagesList") as String;
      return imagesJson;
    }
    List imagesList = await ImagesController().getImagesFirebase(userName);
    ...
    return FirebaseFirestore.instance
        .runTransaction((transaction) async {
          DocumentSnapshot snapshot =
              await transaction.get(users.doc('${user.name}'));
          ...
          transaction.update(
              users.doc('${user.name}'), {'imagesQtt': newFollowerCount});
